...
{
    public partial class mdiMainCopyCopy2 : DevExpress.XtraBars.FluentDesignSystem.FluentDesignForm
    {
        ...
        public mdiMainCopyCopy2(string userName)
        {
            InitializeComponent();
            InitializeTileGroup();
        ...
        private void InitializeChartControl4()
        {
            try
            {
                ...
                string query = @"SELECT [Salesman Code], SUM([Total Amount]) AS TotalSales 
                         FROM [NYCO].[dbo].[AR_Tran_Main_tbl]
        ...
        private void InitializeChartControl3()
        {
            try
            {
                ...
                double totalSales = GetTotalAmount(GetChartSalesAndCollectionData("Ri"));
                double totalCollection = GetTotalAmount(GetChartSalesAndCollectionData("RR"));
        ...
        private void InitializeSalesAndCollectionChart()
        {
            try
            {
                ...
                // Create two series for the chart
                DevExpress.XtraCharts.Series seriesSales = new DevExpress.XtraCharts.Series("Sales", DevExpress.XtraCharts.ViewType.Bar);
                DevExpress.XtraCharts.Series seriesCollection = new DevExpress.XtraCharts.Series("Collection", DevExpress.XtraCharts.ViewType.Bar);
        ...
        DataTable GetChartSalesAndCollectionData(string transactionType)
        {
            DataTable dataTable = new DataTable();
            ...
            try
            {
                ...
                using (SqlConnection connection = new SqlConnection(System.Configuration.ConfigurationManager.ConnectionStrings["connection_string"].ConnectionString))
                {
                    ...
                    string query = $@"SELECT SUM([Total Amount]) AS [Total Amount], MONTH([Document Date]) AS [Extracted Month]
                                FROM [NYCO].[dbo].[AR_Tran_Main_tbl]
                    ...
                    using (SqlCommand command = new SqlCommand(query, connection))
                    {
                        // Set the parameter for the date 2 years ago
                        command.Parameters.AddWithValue("@TwoYearsAgo", twoYearsAgo);
